...
GIT_PROMPT_CLEAN_ICON="%{$fg_bold[white]%}(%{$fg_bold[green]%}✓%{$fg_bold[white]%})"
GIT_PROMPT_DIRTY_ICON="%{$fg_bold[white]%}(%{$fg_bold[red]%}✗%{$fg_bold[white]%})"
...
function git_prompt() {
  ...
  if [ "$branch" = '' ]; then
    ...
    if branch_is_dirty; then
      echo "$GIT_PROMPT_PREFIX$GIT_PROMPT_DIRTY$branch$GIT_PROMPT_DIRTY_ICON$GIT_PROMPT_SUFFIX"
    else
      echo "$GIT_PROMPT_PREFIX$GIT_PROMPT_CLEAN$branch$GIT_PROMPT_CLEAN_ICON$GIT_PROMPT_SUFFIX"
    fi
  fi
